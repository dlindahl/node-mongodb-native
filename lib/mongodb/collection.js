...
Collection.prototype.update = function update (selector, document, options, callback) {
  ...
  if(errorOptions) {    
    ...
    // Execute command with safe options (rolls up both command and safe command into one and executes them on the same connection)
    this.db.executeCommand(updateCommand, commandOptions, function (err, result) {
      var error = result && result.documents;
      if(!callback) return;      
      
      if (err) {
        callback(err);
      } else if (error[0].err, result) {
        callback(self.db.wrap(error[0], null, result));
      } else {
        callback(null, error[0].n, result);
      }      
    });    
  } else {
